...
import { api } from '../api';
import { Search, Pencil, Trash2, X, LayoutGrid, List, AlertTriangle, CheckSquare, Square, User } from 'lucide-react';
...
export default function CustomersPage({ onLogout }: CustomersProps) {
  // Data & Global states
  ...
  const fetchCustomers = async () => {
    try {
      ...
      setSelectedIds((prev) => prev.filter((id) => loadedData.some((item: Customer) => item.id === id)));
    } catch (err: any) {
      ...
      if (err.response?.status === 401) {
        ...
        setError(
          ...
          'Could not load customer records. Check server connection.'
        );
      }
    } finally {
      setLoading(false);
  ...
  const filteredItems = items.filter((item) => {
    const custName = item.name ? item.name.toLowerCase() : '';
    const custPhone = item.phone ? item.phone.toLowerCase() : '';
    const custEmail = item.email ? item.email.toLowerCase() : '';
    const query = search.toLowerCase();

    return custName.includes(query) || custPhone.includes(query) || custEmail.includes(query);
  });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingItem) {
        await api.put(`/customers/${editingItem.id}`, payload);
      } else {
  ...
  const confirmDeleteSingle = (item: Customer) => {
    setConfirmModal({
      isOpen: true,
      title: 'Delete Customer Record',
      message: `Are you sure you want to delete customer "${item.name}"? This action cannot be undone.`,
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="ticket-font uppercase text-2xl sm:text-3xl leading-none text-[#1c1917]">Customers</h1>
          <p className="text-sm text-gray-500 mt-2">Manage customer records, contact info, and addresses.</p>
        </div>
        <button
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 font-semibold text-gray-900">
                        {/* Person Icon before Customer Name */}
                        <div className="flex items-center gap-2">
                          <User className="w-4 h-4 text-slate-400 shrink-0" />
                          <span>{item.name}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-gray-600 price-font">{item.phone || '-'}</td>
                      <td className="px-4 py-3 text-gray-600">{item.email || '-'}</td>
                      <td className="px-4 py-3 text-gray-600 truncate max-w-xs">{item.address || '-'}</td>
                  ...
                  <div className="p-4 flex-1 space-y-2">
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        {/* Person Icon Badge & Name in Card View */}
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            {item.phone && <span className="text-[11px] text-slate-500 price-font block">{item.phone}</span>}
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            <div>
              {search ? (
                <>
                  ...
                  <span className="price-font font-semibold text-[#1c1917]">{items.length}</span> customers
                </>
              ) : (
                <>
                  <span className="price-font font-semibold text-[#1c1917]">{items.length}</span> customers total
                </>
              )}
            </div>
            {selectedIds.length > 0 && (
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-fadeIn">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden flex flex-col border border-gray-100">
            <div className="px-6 py-4 bg-[#1c1917] border-b-2 border-[#c2621f] flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-400 mt-1.5">Manage customer contact details</p>
                </div>
              </div>
              <button
            ...
            <form id="customer-form" onSubmit={handleSave} className="p-5 space-y-3.5 overflow-y-auto">
              <div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="VIP preferences, allergies, etc."
                />
              </div>
            </form>
